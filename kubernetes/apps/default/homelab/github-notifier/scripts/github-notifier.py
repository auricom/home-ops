    ...
    try:
        for repo_config in conf["repositories"]:
            ...
            with conn:
                ...
                # Convert database timestamp string to datetime for comparison
                current_date = None
                if current and current[0]:
                    current_date = datetime.strptime(current[0], "%Y-%m-%d %H:%M:%S")

                if not current_date or release_date > current_date:
                    conn.execute("""
                        INSERT OR REPLACE INTO github_releases (repo_name, latest_release, release_date)
                        VALUES (?, ?, ?)
                    """, (repo_name, latest_tag, release_date.strftime("%Y-%m-%d %H:%M:%S")))
