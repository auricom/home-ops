...
`just talos render-config <node>` builds the final machine config as a native multi-document YAML
stream from four layers:

```
cluster.yaml.j2
networking.yaml.j2
nodes/<node>.yaml.j2
controlplane.yaml.j2 | workers.yaml.j2
```

Each layer passes through `minijinja-cli` (strict Jinja templating; the schematic ID arrives as a
`-D` define) and `op inject` (1Password secret resolution). Talos consumes the resulting
multi-document stream directly; this avoids offline patching of base64-encoded certificate data.
